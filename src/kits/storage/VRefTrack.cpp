...
_caller_str(const void* caller, char* out, size_t outSize)
{
	...
	Dl_info info;
	if (dladdr((void*)caller, &info) != 0 && info.dli_sname != NULL) {
		std::ptrdiff_t off = (const char*)caller - (const char*)info.dli_saddr;
		snprintf(out, outSize, "%s+0x%lx", info.dli_sname, (long)off);
	} else {
