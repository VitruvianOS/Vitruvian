				 ...
				 const void *buffer, size_t len)
{
	...
	if (!attr || !buffer)
		return B_BAD_VALUE;
	ssize_t result = BPrivate::Storage::write_attr(fFd, attr, type, offset, buffer, len);
	return result;
}
				...
				void *buffer, size_t len) const
{
	...
	if (!attr || !buffer)
		return B_BAD_VALUE;
	ssize_t result = BPrivate::Storage::read_attr(fFd, attr, type, offset, buffer, len );
	return result;
}
