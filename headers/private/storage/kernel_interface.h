...
/*! \brief Similar to the first version, aside from that an error code is
	...
	parameter. */
// TODO
status_t dup(int32 file, int32& result);
...
status_t open_dir(const char *path, int &result);
...
int32 read_dir(int dir, DirEntry *buffer, size_t length,
				int32 count = INT_MAX);

/*! Rewindes the directory to the first entry in the list. */
status_t rewind_dir(int dir);
...
status_t find_dir(int dir, const char *name, DirEntry *result,
				   size_t length);

/*! Calls the other version of BPrivate::Storage::find_dir() and stores the results
	in the given entry_ref. */
status_t find_dir(int dir, const char *name, entry_ref *result);
...
/*! Creates a duplicated of the given directory and places it in result if successful,
	...
	unsuccessful. */
status_t dup_dir(int32 dir, int32 &result);
