...
class BluetoothStatus : public BApplication {
	public:
		BluetoothStatus();
		virtual	~BluetoothStatus();
...
const char* kSignature = "application/x-vnd.Haiku-BluetoothStatus";
const char* kDeskbarSignature = "application/x-vnd.Be-TSKB";
const char* kDeskbarItemName = "BluetoothStatus";
...
void
BluetoothStatus::ArgvReceived(int32 argc, char** argv)
{
	if (argc <= 1)
		return;
	...
	if (strcmp(argv[1], "--help") == 0
		...
		str.ReplaceFirst("%appname%", B_TRANSLATE_SYSTEM_NAME("BluetoothStatus"));
		puts(str.String());
...
BluetoothStatus::ReadyToRun()
{
	...
	if (isDeskbarRunning && !isInstalled) {
		...
		text.ReplaceFirst("%appname%", B_TRANSLATE_SYSTEM_NAME("BluetoothStatus"));
	...
	BWindow* window = new BluetoothStatusWindow();
	window->Show();
}


void
BluetoothStatus::AboutRequested()
{
...
void
BluetoothStatus::_InstallReplicantInDeskbar()
{
	image_info info;
	entry_ref ref;
